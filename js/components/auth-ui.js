...
const AuthUI = (function() {
    ...
    // ===== ИСПРАВЛЕННАЯ ФУНКЦИЯ ПОКАЗА ОШИБКИ (XSS защита) =====
    ...
    function attachModalEvents() {
        ...
        // Отправка форм с debounce
        const loginBtn = document.getElementById('loginSubmitBtn');
        if (loginBtn) {
            loginBtn.addEventListener('click', Utils.debounce(handleLogin, 1000));
        }
        
        const registerBtn = document.getElementById('registerSubmitBtn');
        if (registerBtn) {
            registerBtn.addEventListener('click', Utils.debounce(handleRegister, 1000));
        }
